// netlify/functions/api.js  (ESM, una sola función que maneja TODO Cliente/Público)
...
function readURL(event) {
  ...
  const qs = event.queryStringParameters
    ...
    : "";
  return new URL(`http://x${event.path}${qs}`);
}
...
async function opClientRequestsList(event) {
  ...
  try {
    ...
    return J(200, { ok: true, items: rows.map(r => ({
      id: r.id,
...
async function opClientRequestsUpsert(event) {
  ...
  try {
    ...
    if (/invalid input syntax for type uuid/i.test(msg))
      return J(400, { ok: false, error: "UUID inválido" });
    if (/invalid input value for enum service_type/i.test(msg))
      return J(400, { ok: false, error: "service_kind inválido" });
    if (/value for domain lang_code/i.test(msg))
      return J(400, { ok: false, error: "Idioma inválido" });
...
async function opClientRequestsStatus(event) {
  ...
  try {
    ...
    const msg = String(e?.message || e);
    if (/invalid input value for enum request_status/i.test(msg))
      return J(400, { ok: false, error: "Estado inválido" });
    if (/Transición de estado no permitida/i.test(msg))
      return J(400, { ok: false, error: msg });
...
/* =========================
   Cliente: Adjuntos (solo listar; subida en upload.js)
========================= */
async function opClientAttachmentsList(event) {
  ...
  try {
    ...
    const owner = await client.query(
      `SELECT 1 FROM ${SCHEMA}.requests WHERE id=$1::uuid AND client_id=$2::uuid`,
      [request_id, uid]
    );
